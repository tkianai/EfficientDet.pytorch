...
def do_train(
    ...
    for epoch in range(start_epoch, max_epoch):
        for iteration, (images, targets) in enumerate(train_dataloader):
            ...
            optimizer.step()
            lr_scheduler.step(losses_reduced.item())

            batch_time = time.time() - end
            end = time.time()
            meters.update(time=batch_time, data=data_time)

            eta_seconds = meters.time.global_avg * (max_iter - iteration + (max_epoch - epoch) * max_iter)
            ...
            '''
            if val_dataloader is not None and test_period > 0 and epoch % test_period == 0:
    ...
    logger.info(
        "Total training time: {} ({:.4f} s / it)".format(
            total_time_str, total_training_time / (max_iter * max_epoch)
        )
    )
